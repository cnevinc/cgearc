...
<?php
	echo "Hi! ".$_SESSION['username'] ." [<a href=logout.php>登出</a>][<a href=home.php>回到主頁</a>][<a href=manageOrder.php>回到訂單列表</a>]<BR>";

	foreach($_POST as $key => $value){
		//echo "$key => $value <BR>";
	}
	if (isAdmin()){
		$sSQL = "select sum(stp_item_count) as incart , sum(quot_amount*stp_item_count) as check_sum from order_line where act_amount!=-1 and header_id >= 172 and header_id!=173 and header_id in (select header_id from  order_header where order_status=3 and batch_id=9 )";
			$items_result = mysql_query($sSQL); // table 必須至少有一筆資料
			while($r = mysql_fetch_array($items_result)) {
			echo "incart:[".$r['incart']."] . check_sum[".$r['check_sum']."]";
		}
	...
	$sSQL = "UPDATE order_header SET
			...
			update_date = NOW()  ,
			domestic_exp =  N'".mysql_real_escape_string($_POST['domestic_exp'])."',
			balance_due = N'".mysql_real_escape_string($_POST['balance_due'])."'
			";
	...
	if ($_POST['itemID']=="" and $_GET['header_id']==""){
		echo "頁面錯誤,請回到<a href=home.php>主頁面</a>";
		exit;
	}
	if ($_POST['status']=="0" and ($_POST['bank_name_1']=="" or $_POST['bank_code_1']=="" or $_POST['bank_last5_1']=="" or $_POST['bank_amount_1']=="")){
		echo "<font color=red>頭款必填欄位未填寫,變更未儲存</font><BR>";
		if (isAdmin()) {	mysql_query($sSQL) or die (mysql_error());	}
	...
	if ($_POST['itemID']) {	$header_id = $_POST['itemID']; }
	if ($_GET['header_id']) {	$header_id = $_GET['header_id']; }
	$sSQL = "SELECT u.username,h.* from order_header h , users u where h.user_id = u.ID and  header_id = ".mysql_real_escape_string($header_id);
...
團員<input type=text  value='<?=$row['username']?>' readonly size=<?=strlen($row['username'])?>>
...
?>
3.STP <?=$batch_no?>團尾款明細:<font color=red><?=$batch_remark?></font>
<BR>
分攤總覽
<table border=1>
...
<?php
	...
	$balance_due = $sumTotalNTD + $row['domestic_exp'] - $row['bank_amount_1'];
?>

<tr><td>金額總計:</td><td><?=$sumItemUSD?></td><td><?=$sumStp_item_count?></td><td><?=$sumItemNTD?></td><td><?=$sumFreightNTD?></td><td><?=$sumCustomNTD?></td><td><?=$sumFeeNTD?></td><td><?=$sumTotalNTD?></td></tr>
<tr bgcolor=#FFA500><td colspan=7>+國內運費(5kg以下80;5-10KG100;10KG以上120)</td><td><input type=text size=3 name=domestic_exp value='<?= $row['domestic_exp']?>'>(必填)</td></tr>
<tr><td colspan=7>-已付金額:</td><td><?= $row['bank_amount_1']?></td></tr>
<tr><td colspan=7>尾款:</td><td><input type=hidden value=<?=$balance_due?> name=balance_due ><?=$balance_due?> </td></tr>
</table>
<BR><BR>
<?php
...
?>
4.請填取貨資訊<BR> <font color=red>由於7-11現在所有包裝都要用紙箱,還要另外收費. 所以現在暫時只提供面交與郵寄的選項. 謝謝!</font>
 <table border=1>
 <tr>
	<td><font color=red>交貨方式(必填)</font></td> 
	<td>預定面交日期(面交才填)</td> 
	<td><!--7-11店號&nbsp;&nbsp;<a href='http://www.google.com.tw?q=711門市' target=_blank>查店號</a>-->暫不提供店到店</td>
	<td>(郵寄才填,務必寫郵遞區號)</td>
	<td><font color=red>收件人資料(手機/姓名必填)</font></td>
 </tr>
 <tr>
	<td><select name=delivery_mothod >
			...
			<!--<option value=711店到店>7-11店到店</option>-->
			<option value=郵局>郵局</option>
		</select></td> 
	<td>如:7/15 		<input maxlength=20 name=delivery_time  size=10 value='<?php echo $row['delivery_time'];?>' > </td>
	<td><!--如:990433/店名				<input maxlength=14 size=10  name=delivery_store_name value='<?php echo $row['delivery_store_name'];?>' readonly >--></td>
	<td>地址: 					<input name=delivery_address value='<?php echo $row['delivery_address'];?>'  ></td>
	<td>手機/姓名/備註: 		<input maxlength=40 name=delivery_addressee value='<?php echo $row['delivery_addressee'];?>'  >	</td>
 </tr>
 </table>
 
 <BR><BR>

5.<font color=red>尾款</font>資料: 無論面交,店到店,郵寄,此處都要填寫,以便對帳. 若尾款為負數,所有欄位都請填寫0, 並請於最下方"訂單備註" 註明您要將餘款1.留至下次出團 2.裝在包裝中寄回 3.完整帳號轉帳退款(需扣手續費13元)
<table border=1>
 <tr>
	<td><font color=red>匯款人(必填)</font></td> 
	<td><font color=red>匯款銀行(必填)</font></td>
