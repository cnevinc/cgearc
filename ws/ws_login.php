...
if (isset($_POST['submit'])) { // if form has been submitted
	// makes sure they filled it in
 	if(!$_POST['username'] | !$_POST['pass']) {
		$arr = array ('message'=>'Please file every required fields');
	...
	if ($check2 == 0) {
		$arr = array ('message'=>'No such person... please register first!');
		echo json_encode($arr);
 	}
	...
	while($info = mysql_fetch_array( $check )) {
		...
		if ($_POST['pass'] != $info['password']) {
			...
			$arr = array ('success'=>true,'user_id'=>$info['ID'],'username'=>$info['username'],'token'=>12345,'message'=>12345);
			echo json_encode($arr);
		} 
	} 
