...
<?PHP
	$batch_result = mysql_query("SELECT * FROM order_batch WHERE batch_id =3 "); // table 必須至少有一筆資料
...
<?php
	$SQL = "SELECT 	l.*,
			...
			AND		l.act_amount>0 
			AND 	l.batch_id =3 
			ORDER BY l.stp_item_name";
	
	$items_result = mysql_query($SQL); // table 必須至少有一筆資料
	$est_amount=0;
...
<?PHP
	$batch_result = mysql_query("SELECT * FROM order_batch WHERE batch_id = 1 "); // table 必須至少有一筆資料
	while($batch_r = mysql_fetch_array($batch_result)) {
		$batch_bill_usd 	= $batch_r['batch_bill_usd']; 
		$batch_bill_ntd 	= $batch_r['batch_bill_ntd'];
		$unitCurrency 		= ceil($batch_bill_ntd / $batch_bill_usd*10000)/10000;
