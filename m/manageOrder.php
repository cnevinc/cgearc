...
<?php
	$result = mysql_query("select sum(stp_item_count) as sum_c from order_line where batch_id = 3 and act_amount<>0 ") or die(mysql_error());
	$row = mysql_fetch_assoc($result);
?>
本團已經下定[<?=$row['sum_c']?>]件物品
...
while($row = mysql_fetch_array($result)) {
	...
	$order_status =	"<form action=editOrderInfo.php?header_id=".$row['header_id']." method=get><input type ='submit'  value='訂單明細'></form>";
